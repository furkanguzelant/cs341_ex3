...
async function main() {
  ...
  const draw_triangle_with_offset = regl({
    ...
    vert: /*glsl*/ `
		...
		void main() {
			...
			vec2 pos = position + mouse_offset;
  ...
  const draw_triangle_with_transform = regl({
    ...
    vert: /*glsl*/ `
		...
		void main() {
			...
			vec2 pos = (mat_transform * vec4(position, 0, 1)).xy;
			gl_Position = vec4(pos, 0, 1);
		}`,
  ...
  regl.frame((frame) => {
    ...
    /*
		...
		*/
    let translation = mat4.fromTranslation(mat4.create(), [0.5, 0, 0]);
    let rotation = mat4.fromZRotation(
      mat4.create(),
      (sim_time * 30 * Math.PI) / 180
    );
    draw_triangle_with_transform({
      mat_transform: mat4_matmul_many(mat4.create(), rotation, translation),
      color: [0, 1, 0],
    });

    draw_triangle_with_transform({
      mat_transform: mat4_matmul_many(mat4.create(), translation, rotation),
      color: [1, 0, 0],
    });

    // You can write whatever you need in the debug box
    debug_text.textContent = `
Hello! Sim time: ${sim_time.toFixed(2)} s | Mouse offset: ${vec_to_string(
      mouse_offset,
      2
    )}
`;
