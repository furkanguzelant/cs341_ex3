...
async function load_resources(regl) {
  ...
  resource_promises["earth_clouds.jpg"] = load_texture(
    regl,
...
async function main() {
  ...
  function set_selected_planet(name) {
    ...
    frame_info.cam_distance_factor =
      (3 * scene_info.actors_by_name[name].size) / cam_distance_base;
    update_cam_transform(frame_info);
  }
  ...
  regl.frame((frame) => {
    ...
    // Calculate view matrix, view centered on chosen planet
    {
      ...
      const selected_planet_model_mat =
        scene_info.actors_by_name[selected_planet_name].mat_model_to_world;
      const selected_planet_position = mat4.getTranslation(
        [0, 0, 0],
        selected_planet_model_mat
      );
      ...
      mat4_matmul_many(
        mat_view,
        mat_turntable,
        selected_planet_translation_mat
      );
    }
...
Camera: angle_z ${(frame_info.cam_angle_z / deg_to_rad).toFixed(1)}, angle_y ${(
      frame_info.cam_angle_y / deg_to_rad
    ).toFixed(1)}, distance ${(
      frame_info.cam_distance_factor * cam_distance_base
    ).toFixed(1)}
